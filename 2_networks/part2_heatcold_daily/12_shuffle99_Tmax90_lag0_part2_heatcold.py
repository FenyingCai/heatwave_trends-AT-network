...
ds01 = xr.open_dataset("/public/home/fcai/extreme1_AT/NC2_1980_2010/1_threshold/era5_Tave_th10_JJA_1981_2010.nc")
...
t_False = np.zeros((44, 92, np.shape(t0)[1],np.shape(t0)[2]))
for iyear in range(44):
  t_False[iyear,:,:,:] = np.array(t[iyear,:,:,:]) <= np.array(t_th10)
del t_th10, t  #; print(np.sum(t_False)); print(np.size(t_False))
...
t_False_days = np.sum(np.sum(t_False2, axis=1), axis=0)
print(" Cold days  ",np.min(t_False_days), np.max(t_False_days))
...
for alat in range(np.shape(t0)[1]):
  ...
  for alon in range(np.shape(t0)[2]):
    ...
    for b_daysF in range(0,800):
      concurrent_days = matrix99[a_daysT, b_daysF]
      matrix4D_99[alat,alon,:,:][np.where(t_False_days[:,:]==b_daysF)] = concurrent_days
      del b_daysF, concurrent_days
    del a_daysT
...
ds0.to_netcdf("/public/home/fcai/extreme1_AT/NC2_1980_2010/2_networks/part2_heatcold_daily/shuffle99_Tmax90_Tave10_lag0_part2_heatcold.nc")
ds0.close()
