...
PC_EE = np.sum(np.sum(np.array(t_True2[:,:,45:65+1,25:55+1]),axis=3),axis=2)
print('PC_EE', np.shape(PC_EE), np.max(PC_EE), np.min(PC_EE), np.nanpercentile(PC_EE, 90))



##--##--##--##--  heatwave grid cells > 90th  --##--##--##--##
PC_index = np.array(np.where(PC_EE>=np.nanpercentile(PC_EE, 90)))
print('   90% percentile,   ', np.nanpercentile(PC_EE, 90))
...
##--##--##--##--   Composite   --##--##--##--##
t_EE = np.zeros((np.shape(PC_index)[1], np.shape(t0)[1],np.shape(t0)[2]))
u_EE = np.zeros((np.shape(PC_index)[1], np.shape(t0)[1],np.shape(t0)[2]))
v_EE = np.zeros((np.shape(PC_index)[1], np.shape(t0)[1],np.shape(t0)[2]))
h_EE = np.zeros((np.shape(PC_index)[1], np.shape(t0)[1],np.shape(t0)[2]))
for i in range(np.shape(PC_index)[1]):
  t_EE[i,:,:] = t[PC_index[0,i],PC_index[1,i],:,:].copy()
  u_EE[i,:,:] = u[PC_index[0,i],PC_index[1,i],:,:].copy()
  v_EE[i,:,:] = v[PC_index[0,i],PC_index[1,i],:,:].copy()
  h_EE[i,:,:] = h[PC_index[0,i],PC_index[1,i],:,:].copy()

day1 = np.linspace(1,np.shape(PC_index)[1],np.shape(PC_index)[1])
t_EE = xr.DataArray(data=t_EE.data, dims=['day','lat','lon'], coords={'day':day1, 'lat':lat.data, 'lon':lon.data})
u_EE = xr.DataArray(data=u_EE.data, dims=['day','lat','lon'], coords={'day':day1, 'lat':lat.data, 'lon':lon.data})
v_EE = xr.DataArray(data=v_EE.data, dims=['day','lat','lon'], coords={'day':day1, 'lat':lat.data, 'lon':lon.data})
h_EE = xr.DataArray(data=h_EE.data, dims=['day','lat','lon'], coords={'day':day1, 'lat':lat.data, 'lon':lon.data})
...
t_diff = np.mean(t_EE.data, axis=0) - np.mean(t_4D.data, axis=0)
u_diff = np.mean(u_EE.data, axis=0) - np.mean(u_4D.data, axis=0)
v_diff = np.mean(v_EE.data, axis=0) - np.mean(v_4D.data, axis=0)
h_diff = np.mean(h_EE.data, axis=0) - np.mean(h_4D.data, axis=0)
...
ds2 = xr.Dataset(data_vars=dict(t_EE=t_EE, u_EE=u_EE, v_EE=v_EE, h_EE=h_EE,\
                                t=t_4D, u=u_4D, v=v_4D, h=h_4D,\
                                t_diff=t_diff, u_diff=u_diff, v_diff=v_diff, h_diff=h_diff))
ds2.to_netcdf("/public/home/fcai/extreme1_AT/NC2_1980_2010/2_networks/case_eastern_Europe/era5_EE_composite_T2m_UVH500.nc")
ds2.close()
del t_EE, u_EE, v_EE, h_EE, t_4D, u_4D, v_4D, h_4D
...
##--##--##--##--   Composite   --##--##--##--##
u_EE = np.zeros((np.shape(PC_index)[1], 5, np.shape(t0)[1],np.shape(t0)[2]))
v_EE = np.zeros((np.shape(PC_index)[1], 5, np.shape(t0)[1],np.shape(t0)[2]))
h_EE = np.zeros((np.shape(PC_index)[1], 5, np.shape(t0)[1],np.shape(t0)[2]))
for i in range(np.shape(PC_index)[1]):
  u_EE[i,:,:,:] = u[PC_index[0,i],PC_index[1,i],:,:,:].copy()
  v_EE[i,:,:,:] = v[PC_index[0,i],PC_index[1,i],:,:,:].copy()
  h_EE[i,:,:,:] = h[PC_index[0,i],PC_index[1,i],:,:,:].copy()
...
day1 = np.linspace(1,np.shape(PC_index)[1],np.shape(PC_index)[1])
u_EE = xr.DataArray(data=u_EE.data, dims=['day','level','lat','lon'], coords={'day':day1, 'level':level, 'lat':lat.data, 'lon':lon.data})
v_EE = xr.DataArray(data=v_EE.data, dims=['day','level','lat','lon'], coords={'day':day1, 'level':level, 'lat':lat.data, 'lon':lon.data})
h_EE = xr.DataArray(data=h_EE.data, dims=['day','level','lat','lon'], coords={'day':day1, 'level':level, 'lat':lat.data, 'lon':lon.data})
...
ds2 = xr.Dataset(data_vars=dict(u_EE=u_EE, v_EE=v_EE, h_EE=h_EE,\
                                u=u_4D, v=v_4D, h=h_4D,\
                                u_diff=u_diff, v_diff=v_diff, h_diff=h_diff))
ds2.to_netcdf("/public/home/fcai/extreme1_AT/NC2_1980_2010/2_networks/case_eastern_Europe/era5_EE_composite_UVH_5levels.nc")
ds2.close()
