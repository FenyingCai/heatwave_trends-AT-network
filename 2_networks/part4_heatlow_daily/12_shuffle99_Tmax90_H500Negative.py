...
h_Negative = np.zeros((44, 92, np.shape(t0)[1],np.shape(t0)[2]))
for iyear in range(44):
  h_ano = np.array(h[iyear,:,:,:]) - np.array(h_clm)
  h_Negative[iyear,:,:,:] = np.array(h_ano) <= (-1.0)* np.array(h_std); del h_ano
...
h_Negative_days = np.sum(np.sum(h_Negative, axis=1), axis=0)
print(" H Negative days  ",np.min(h_Negative_days), np.max(h_Negative_days))
del t_True2, h_Negative
...
for blat in range(np.shape(t0)[1]):
  ...
  for blon in range(np.shape(t0)[2]):
    b_days = int(h_Negative_days[blat,blon])
...
ds0.to_netcdf("/public/home/fcai/extreme1_AT/NC2_1980_2010/2_networks/part4_heatlow_daily/shuffle99_Tmax90_H500Negative_lag0.nc")
ds0.close()
