...
for iyear in range(44):
  ...
  h_Negative = np.zeros((92, np.shape(t0)[1],np.shape(t0)[2]))
  h_Negative[:,:,:] = np.array(h_ano) < (-1.0) * np.array(h_std); del h_ano
  print(" High  pressure  days ",np.sum(h_Negative), np.size(h_Negative))
  ...
  for alat in range(np.shape(t0)[1]):
    ...
    for alon in range(np.shape(t0)[2]):
      ...
      concurrent_days = np.sum(t_True_3D[:,:,:]+h_Negative[:,:,:]==2,axis=0)
  ...
  ds0.to_netcdf("/home/ys17-23/cai_fy/data4_heatwaves/NCC2_1980_2010/2_networks/part4_heatlow_daily/Networks_Tmax90_H500Negative_lag0_"+str(iyear+1979)+".nc")
  ds0.close()
